   ...
   rightButtonTitle:(NSString *)rigthTitle
{
    if (self = [super init]) {
        ...
        CGSize textSize = [content boundingRectWithSize:CGSizeMake(100, 0)
                                            ...
                                            context:nil].size;
        self.alertContentLabel = [[UILabel alloc] initWithFrame:CGRectMake((kAlertWidth - contentLabelWidth) * 0.5, CGRectGetMaxY(self.alertTitleLabel.frame), contentLabelWidth, textSize.height)];
...
#define kButtonBottomOffset 10.0f
        CGFloat BtnY = CGRectGetMaxY(self.alertContentLabel.frame);
        if (!leftTitle) {
            //kAlertHeight - kButtonBottomOffset - kButtonHeight
            rightBtnFrame = CGRectMake((kAlertWidth - kSingleButtonWidth) * 0.5, BtnY, kSingleButtonWidth, kButtonHeight);
            self.rightBtn = [UIButton buttonWithType:UIButtonTypeCustom];
            self.rightBtn.frame = rightBtnFrame;
        }else {
            //kAlertHeight - kButtonBottomOffset - kButtonHeight
            leftBtnFrame = CGRectMake((kAlertWidth - 2 * kCoupleButtonWidth - kButtonBottomOffset) * 0.5, BtnY, kCoupleButtonWidth, kButtonHeight);
            rightBtnFrame = CGRectMake(CGRectGetMaxX(leftBtnFrame) + kButtonBottomOffset, BtnY, kCoupleButtonWidth, kButtonHeight);
        ...
        CGFloat alertHeight = CGRectGetMaxY(self.rightBtn.frame)+3;
...
- (void)show
{
    CGFloat AlertHeight = CGRectGetMaxY(self.rightBtn.frame);
